...
void lt_dump_script (FILE* f)
{
  ...
  fputs ("    echo \"http-server.exe:sample/http-server:$LINENO: libtool wrapper (GNU libt", f);
  ...
  fputs ("  for lt_arg\n", f);
  fputs ("  do\n", f);
  fputs ("    $ECHO \"http-server.exe:sample/http-server:$LINENO: newargv[$lt_dump_args_N]", f);
  fputs (": $lt_arg\"\n", f);
  ...
  fputs ("\n", f);
  fputs ("      if test -n \"$lt_option_debug\"; then\n", f);
  fputs ("        $ECHO \"http-server.exe:sample/http-server:$LINENO: newargv[0]: $progdir", f);
  fputs ("/$program\" 1>&2\n", f);
